...
static int avs_rt5682_codec_init(struct snd_soc_pcm_runtime *runtime)
{
	...
	pins = devm_kmemdup_array(dev, card_jack_pins, num_pins,
...
static void avs_rt5682_codec_exit(struct snd_soc_pcm_runtime *rtd)
{
	snd_soc_component_set_jack(
		snd_soc_dai_to_component(snd_soc_rtd_to_codec(rtd, 0)), NULL, NULL);
}
...
static int avs_rt5682_probe(struct platform_device *pdev)
{
	...
	if (!jack || !card || !card_driver)
		return -ENOMEM;

	if (pdata->obsolete_card_names) {
		snd_soc_card_set_name(card, "avs_rt5682");
	} else {
		card_driver->driver_name = "avs_rt5682";
		snd_soc_card_set_name(card, "AVS I2S ALC5682");
		snd_soc_card_set_long_name(card, "AVS I2S ALC5682");
	}
	card_driver->owner = THIS_MODULE;
	card_driver->suspend_pre = avs_card_suspend_pre;
	card_driver->resume_post = avs_card_resume_post;
