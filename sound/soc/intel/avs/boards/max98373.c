...
static int avs_max98373_probe(struct platform_device *pdev)
{
	...
	if (pdata->obsolete_card_names) {
		snd_soc_card_set_name(card, "avs_max98373");
	} else {
		card_driver->driver_name = "avs_max98373";
		snd_soc_card_set_name(card, "AVS I2S MAX98373");
		snd_soc_card_set_long_name(card, "AVS I2S MAX98373");
	}
	card_driver->owner = THIS_MODULE;
	card_driver->dai_link = dai_link;
	card_driver->num_links = 1;
	card_driver->codec_conf = card_codec_conf;
	card_driver->num_configs = ARRAY_SIZE(card_codec_conf);
