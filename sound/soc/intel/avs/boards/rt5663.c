...
static int avs_rt5663_probe(struct platform_device *pdev)
{
	...
	if (!priv || !card || !card_driver)
		return -ENOMEM;

	if (pdata->obsolete_card_names) {
		snd_soc_card_set_name(card, "avs_rt5663");
	} else {
		card_driver->driver_name = "avs_rt5663";
		snd_soc_card_set_name(card, "AVS I2S ALC5663");
		snd_soc_card_set_long_name(card, "AVS I2S ALC5663");
	}
