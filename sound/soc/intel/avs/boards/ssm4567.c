...
static int avs_ssm4567_probe(struct platform_device *pdev)
{
	...
	if (pdata->obsolete_card_names) {
		snd_soc_card_set_name(card, "avs_ssm4567");
	} else {
		card_driver->driver_name = "avs_ssm4567";
		snd_soc_card_set_name(card, "AVS I2S SSM4567");
		snd_soc_card_set_long_name(card, "AVS I2S SSM4567");
	}
