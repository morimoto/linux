...
static int mc_probe(struct platform_device *pdev)
{
	...
	card_driver->late_probe = sof_sdw_card_late_probe;
	card_driver->add_dai_link = sof_sdw_add_dai_link;

	snd_soc_card_set_name(card, "soundwire");
	...
	ret = sof_card_dai_links_create(card, card_driver);
	if (ret < 0)
		return ret;
