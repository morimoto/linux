...
int asoc_sdw_cs47l47_hs_rtd_init(struct snd_soc_pcm_runtime *rtd, struct snd_soc_dai *dai)
{
	...
	snd_soc_card_set_components(card, devm_kasprintf(dev, GFP_KERNEL, "%s hs:cs47l47",
...
int asoc_sdw_cs47l47_dmic_rtd_init(struct snd_soc_pcm_runtime *rtd, struct snd_soc_dai *dai)
{
	...
	snd_soc_card_set_components(card, devm_kasprintf(dev, GFP_KERNEL, "%s mic:cs47l47-dmic",
							 snd_soc_card_components(card)));
	if (!snd_soc_card_components(card))
		return -ENOMEM;
